...
namespace TouchScript.Behaviors.Visualizer
{
    ...
    public class TouchProxy : TouchProxyBase
    {
       ...
       // public Text Text;
        ...
        protected override void updateOnce(TouchPoint touch)
        {
            ...
            stringBuilder.Append(touch.Position.x);
            stringBuilder.Append(" / PosY: ");
            stringBuilder.Append(touch.Position.y);
    ...
    public class TouchProxyBase : MonoBehaviour
    {
        ...
        public virtual void update(TouchPoint touch)
        {
            ...
            string[] tagName = touch.Tags.ToString().Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);

            stringBuilder.Append(tagName[1]);
            
            if (tagName[1].Equals("Object"))
            {
                stringBuilder.Append(" / Rot: ");
                float angle = ((float)touch.Properties["Angle"] * 180) / Mathf.PI;
                if(angle >= 360.0)
                {
                    angle -= (float)360.0;
                }else if(angle <= 0.0)
                ...
                //stringBuilder.Append(((float)touch.Properties["Angle"]*180)/Mathf.PI);
